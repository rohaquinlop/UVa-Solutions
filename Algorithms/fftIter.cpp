...
void fft(vector<cd>& a, bool flag) {
  ...
  while ((1 << numBits) < n) numBits++;

  for (i = 0; i < n; ++i) {
    aux = reverse(i, numBits);
    if (i < aux) swap(a[i], a[aux]);
  }
...
void multiplicarPolinomios(vector<cd>& compA, vector<cd>& compB) {
  ...
  while (n < compA.size() + compB.size()) n <<= 1;
  ...
  for (int i = 0; i < n; ++i) compA[i] *= compB[i];

  fft(compA, true);
}

int main() {
  int i, aux, ac, arr1[] = {4, 3, 6, 5}, arr2[] = {2, 7, 3, 8},
                  arr3[] = {1, 2, 3, 4};
  ...
  for (i = 0; i < pol3.size(); ++i) cout << pol3[i] << " ";
  printf("\n");

  // si los elementos de los vectores representan los coeficientes de dos
  // polinomios, entonces el resultado corresponde a los coeficientes del
  // producto
  for (i = 0; i < pol1.size(); ++i) printf("%d ", (int)round(pol1[i].real()));
  printf("\n");
